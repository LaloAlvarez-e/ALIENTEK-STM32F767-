...
SCB_nERROR SCB_UsageFault__enSetPending(SCB_nMODULE enModuleArg)
{
    ...
    stRegister.uxValue = SCB_SHCSR_USGFAULTPENDED_PENDED;
    enErrorReg = SCB__enWriteRegister(enModuleArg, &stRegister);
...
SCB_nERROR SCB_UsageFault__enClearPending(SCB_nMODULE enModuleArg)
{
    ...
    stRegister.uxMask = SCB_SHCSR_USGFAULTPENDED_MASK;
    stRegister.uptrAddress = SCB_SHCSR_OFFSET;
    stRegister.uxValue = SCB_SHCSR_USGFAULTPENDED_UNPENDED;
    enErrorReg = SCB__enWriteRegister(enModuleArg, &stRegister);
...
SCB_nERROR SCB_UsageFault__enIsActive(SCB_nMODULE enModuleArg, SCB_nBOOLEAN* enStateArg)
{
    ...
    if(SCB_enERROR_OK == enErrorReg)
    {
        stRegister.uxShift = SCB_SHCSR_R_USGFAULTACT_BIT;
        stRegister.uxMask = SCB_SHCSR_USGFAULTACT_MASK;
        stRegister.uptrAddress = SCB_SHCSR_OFFSET;
        enErrorReg = SCB__enReadRegister(enModuleArg, &stRegister);
    }
    if(SCB_enERROR_OK == enErrorReg)
...
{
    SCB_nERROR enErrorReg;
    enErrorReg = SCB_UsageFault__enSetState(enModuleArg, SCB_enSTATE_DIS);
    return (enErrorReg);
}
